 ...
 *
 *	- Supported interrupts for all the streams
 *
 *	- APIs to check transmission status (half completed - completed - error)
 *
 *	- Flexible control of alll interrupts in a stream (except FIFO interrupt)
 *
 */
...
/***** Get the status of transmission (half completed - completed - Error)
 *
 * args: void
 *
 * return: report indicate complete transfer - half transfer - error during transfer
 *
 * EX: DMA_GetTransferState(HALF_TRANSFER_S5, TRANSFER_COMPLETE_S5, TRANSFER_ERROR_S5);
 */
StatusReport_t DMA_GetTransferState(HalfTransCompleteInt halfComp, TransCompleteInt comp, TransError errorInt)
{
	...
	report.half_transfer = DMA_ReadIntFlag(halfComp);
	report.transfer_complete = DMA_ReadIntFlag(comp);
	report.transfer_error = DMA_ReadIntFlag(errorInt);
...
/*
 * 		Functions to enable and disable all interrupts to protect critical sections
 */
